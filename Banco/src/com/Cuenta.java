...
public class Cuenta {
	...
	public Object retirar(Cuenta origen, double monto, int numOperacion) {
		...
		if (origen.getSaldoDisponible() > 0) {
			if ((origen.getSaldoDisponible() - monto) < origen.getMin()) {
				...
				return new Comprobante(new Date(), origen.getNumCuenta(), monto, numOperacion);
			}
		} else {
	...
	public Object depositar(Cuenta destino, double monto,int numOperacion) {
		...
		if (destino.isStatus() && (destino.getSaldoDisponible() + monto) <= destino.getMax()) {
			...
			return new Comprobante(new Date(), destino.getNumCuenta(), monto, numOperacion);
		} else {
	...
	public Object transferir(Cuenta destino, double monto, int numOperacion) {
		...
		if (this.status && destino.isStatus()) {
			...
			if ((this.saldoDisponible - monto) < this.min) {
				...
				destino.setSaldoDisponible(destino.getSaldoDisponible() + monto);				
				return new Comprobante(new Date(), this.numCuenta, monto, numOperacion);
			}
		} else {
			return "Operacion no realizada. Al menos una de las cuentas está inactiva";
		}
	}
