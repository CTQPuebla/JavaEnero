...
public class Cuenta {
	...
	public Object retirar (Cuenta origen, double monto, int numOp) {
//		minimo,maximo y saldoSisponible 
	...
	public Object depositar(Cuenta destino, double monto) {
		...
		//que el monto + saldoDisponivle no sea mayor 
		...
		if(destino.isStatus() && (destino.getSaldoDisponible()+ monto)<= destino.getMax()) {
			...
			return "El deposito excede el maximo permitido de la cuenta ";
	...
	public Object transferir (Cuenta destino, double monto) {
		if(this.status && destino.isStatus()) {
			if((this.saldoDisponible - monto) < this.min) {
				return "	La transferencia dejaeria sin saldo suficiente a la cuenta";
			}else if(destino.getMax() < monto || (destino.getSaldoDisponible() + monto) > destino.getMax()) {
				return "La transferencia exece el maximo permitido en l cuenta";
			}else {
