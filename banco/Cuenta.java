...
public class Cuenta {
	...
	public Object retirar(Cuenta origen, double monto) {
		//minimo, máximo, saldoDisponible
		if(origen.getSaldoDisponible()>0) {
			
			if((origen.getSaldoDisponible()-monto)<origen.getMin()){
				return "El retiro dejaría sin saldo suficiente a la cuenta";
			} else {
	...
	public Object depositar(Cuenta destino, double monto) {
		//status, monto que no sea cero, que no sea mayor al máximo por si solo
		...
		if(destino.isStatus() && (destino.getSaldoDisponible()+monto)<=destino.getMax()) {
			...
			return "La cuenta está inactiva o el deposito excede el máximo permitido en la cuenta" ;
		}
	}
	...
	public Object transferir(Cuenta destino, double monto) {
		if (this.status && destino.isStatus()) {
			if ((this.saldoDisponible - monto)< this.min) {
				return "La transferencia dejaría sin saldo suficiente a la cuenta";
			} else if (destino.getMax()<monto || (destino.getSaldoDisponible()+monto)>destino.getMax()) {
				return "La transferencia excede el máximo permitido en la cuenta";
			}else {
		...
		return "Operación no realizada. Al menos una de las cuentas está inactiva";
	}
